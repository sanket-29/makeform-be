...
const getGroupName = (app) => {
    ...
    if (permitName && !isShortCode(permitName))
        return permitName;
    // Step 3: data object se dhundo — short codes skip karo
    const groupValue = app?.data?.permitName ||
        app?.data?.trade ||
        app?.data?.permitType ||
        app?.data?.category ||
        app?.data?.department ||
...
const getPermitRows = async () => {
    const [applications, permits, payments, feeRecords] = await Promise.all([
        application_1.default.find({}).lean(),
        permit_1.default.find({}).lean(),
        paymentRecord_1.default.find({}).lean(),
        feeRecord_1.default.find({}).lean(),
    ]);
    ...
    permits.forEach((permit) => {
        if (permit.applicationId)
            permitMap.set(String(permit.applicationId), permit);
    });
    payments.forEach((payment) => {
        if (payment.applicationId)
            paymentMap.set(String(payment.applicationId), payment);
    });
    feeRecords.forEach((fee) => {
        if (fee.applicationId)
            feeMap.set(String(fee.applicationId), fee);
    });
    return applications.map((app) => {
        ...
        const applicant = findValue(app, ["applicant", "applicant name", "full name", "name", "owner name", "contact name"]) ||
            (app.data?.name ? String(app.data.name) : "");
        const owner = findValue(app, ["owner", "property owner", "owner name", "owner/agent", "applicant"]) ||
            (app.data?.owner ? String(app.data.owner) : "");
        const siteAddress = findValue(app, [
            "site address",
            "property address",
            "address",
            "location",
        ...
        const mapLotBlock = findValue(app, ["map/lot/block", "map", "lot", "block", "parcel id", "parcel"]);
        const description = findValue(app, [
            "brief description",
            "description",
            "project description",
            "scope of work",
...
const summarizeByType = (rows) => {
    ...
    rows.forEach((row) => {
        ...
        entry.cost += row.cost;
        entry.fees += row.fees;
        if (row.transactionMethod.toLowerCase().includes("cash"))
            entry.cash += row.fees;
        else if (row.transactionMethod.toLowerCase().includes("check"))
            entry.check += row.fees;
        else if (row.transactionMethod.toLowerCase().includes("online") || row.transactionMethod.toLowerCase().includes("card"))
            entry.online += row.fees;
        else if (row.transactionMethod.toLowerCase().includes("waived"))
            entry.waived += row.fees;
        else
            entry.transactions += row.paymentDate ? 1 : 0;
        entry.refund += row.refund;
        map.set(key, entry);
    });
    return Array.from(map.values());
};
const buildChartData = (rows) => Object.entries(rows.reduce((acc, row) => {
    acc[row.typeName] = (acc[row.typeName] || 0) + 1;
    return acc;
}, {})).map(([name, value]) => ({ name, value }));
const buildPaymentSummary = (rows) => {
    const totals = { cash: 0, check: 0, online: 0, waived: 0, refund: 0 };
    rows.forEach((row) => {
        if (row.transactionMethod.toLowerCase().includes("cash"))
            totals.cash += row.fees;
        else if (row.transactionMethod.toLowerCase().includes("check"))
            totals.check += row.fees;
        else if (row.transactionMethod.toLowerCase().includes("online") || row.transactionMethod.toLowerCase().includes("card"))
            totals.online += row.fees;
        else if (row.transactionMethod.toLowerCase().includes("waived"))
            totals.waived += row.fees;
    });
    return [
