...
router.get("/overview", auth_middleware_1.verifyToken, async (req, res) => {
    try {
        ...
        const todayApplications = await application_1.default.countDocuments({
            createdAt: {
                $gte: todayStart,
                $lte: todayEnd
            }
        ...
        const issuedToday = await permit_1.default.countDocuments({
            issueDate: {
        ...
        const pending = await application_1.default.countDocuments({ status: "pending" });
        // 4. Revenue - sum of all paid fees
        const feeRecords = await feeRecord_1.default.find({ paid: true });
        ...
        // }));
        // 6. Signoff Summary
        const signoffsData = await signoff_model_1.default.find({});
        let approved = 0;
        ...
        const applicationsList = await application_1.default.find({});
        const permits = await permit_1.default.find({});
        ...
        const getBucket = (daysAgo) => {
            if (daysAgo === 0)
                return "0 DAYS";
            if (daysAgo >= 1 && daysAgo <= 5)
                return "1-5 DAYS";
            if (daysAgo >= 6 && daysAgo <= 10)
                return "6-10 DAYS";
            return "11+ DAYS";
        };
        const normalizeRawLabel = (label) => label.toString().toLowerCase().replace(/[_\-\s]+/g, " ").trim();
        const findRawValue = (app, keys) => {
        ...
        const departmentBucket = (daysAgo) => {
            if (daysAgo === 0)
                return "0 DAYS";
            if (daysAgo >= 1 && daysAgo <= 5)
                return "1-5 DAYS";
        ...
        const changeRequests = await permit_1.default.find({
